...
function extractorsFactory(extractWeights, paramMappings) {
    ...
    function extractReductionBlockParams(channelsIn, channelsOut, mappedPrefix) {
        var separable_conv0 = extractSeparableConvParams(channelsIn, channelsOut, "".concat(mappedPrefix, "/separable_conv0"));
        var separable_conv1 = extractSeparableConvParams(channelsOut, channelsOut, "".concat(mappedPrefix, "/separable_conv1"));
        var expansion_conv = extractConvParams(channelsIn, channelsOut, 1, "".concat(mappedPrefix, "/expansion_conv"));
        return { separable_conv0: separable_conv0, separable_conv1: separable_conv1, expansion_conv: expansion_conv };
    }
    function extractMainBlockParams(channels, mappedPrefix) {
        var separable_conv0 = extractSeparableConvParams(channels, channels, "".concat(mappedPrefix, "/separable_conv0"));
        var separable_conv1 = extractSeparableConvParams(channels, channels, "".concat(mappedPrefix, "/separable_conv1"));
        var separable_conv2 = extractSeparableConvParams(channels, channels, "".concat(mappedPrefix, "/separable_conv2"));
...
export function extractParams(weights, numMainBlocks) {
    ...
    range(numMainBlocks, 0, 1).forEach(function (idx) {
        middle_flow["main_block_".concat(idx)] = extractMainBlockParams(128, "middle_flow/main_block_".concat(idx));
    });
    ...
    if (getRemainingWeights().length !== 0) {
        throw new Error("weights remaing after extract: ".concat(getRemainingWeights().length));
    }
    return {
