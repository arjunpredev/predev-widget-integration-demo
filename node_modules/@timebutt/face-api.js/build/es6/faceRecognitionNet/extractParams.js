...
function extractorsFactory(extractWeights, paramMappings) {
    function extractFilterValues(numFilterValues, numFilters, filterSize) {
        ...
        if (isFloat(depth)) {
            throw new Error("depth has to be an integer: ".concat(depth, ", weights.length: ").concat(weights.length, ", numFilters: ").concat(numFilters, ", filterSize: ").concat(filterSize));
        }
    ...
    function extractConvParams(numFilterValues, numFilters, filterSize, mappedPrefix) {
        ...
        paramMappings.push({ paramPath: "".concat(mappedPrefix, "/filters") }, { paramPath: "".concat(mappedPrefix, "/bias") });
        return { filters: filters, bias: bias };
    }
    function extractScaleLayerParams(numWeights, mappedPrefix) {
        var weights = tf.tensor1d(extractWeights(numWeights));
        var biases = tf.tensor1d(extractWeights(numWeights));
        paramMappings.push({ paramPath: "".concat(mappedPrefix, "/weights") }, { paramPath: "".concat(mappedPrefix, "/biases") });
        return {
            weights: weights,
            biases: biases
        };
    ...
    function extractConvLayerParams(numFilterValues, numFilters, filterSize, mappedPrefix) {
        var conv = extractConvParams(numFilterValues, numFilters, filterSize, "".concat(mappedPrefix, "/conv"));
        var scale = extractScaleLayerParams(numFilters, "".concat(mappedPrefix, "/scale"));
    ...
    function extractResidualLayerParams(numFilterValues, numFilters, filterSize, mappedPrefix, isDown) {
        ...
        var conv1 = extractConvLayerParams((isDown ? 0.5 : 1) * numFilterValues, numFilters, filterSize, "".concat(mappedPrefix, "/conv1"));
        var conv2 = extractConvLayerParams(numFilterValues, numFilters, filterSize, "".concat(mappedPrefix, "/conv2"));
