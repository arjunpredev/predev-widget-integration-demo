...
var Box = /** @class */ (function () {
    function Box(_box, allowNegativeDimensions) {
        ...
        if (!isRect && !isBbox) {
            throw new Error("Box.constructor - expected box to be IBoundingBox | IRect, instead have ".concat(JSON.stringify(box)));
        }
        var _a = isRect
    ...
    Box.assertIsValidBox = function (box, callee, allowNegativeDimensions) {
        ...
        if (!Box.isRect(box)) {
            throw new Error("".concat(callee, " - invalid box: ").concat(JSON.stringify(box), ", expected object with properties x, y, width, height"));
        }
        if (!allowNegativeDimensions && (box.width < 0 || box.height < 0)) {
            throw new Error("".concat(callee, " - width (").concat(box.width, ") and height (").concat(box.height, ") must be positive numbers"));
        }
    };
