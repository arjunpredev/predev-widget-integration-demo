...
function extractorsFactory(weightMap, paramMappings) {
    ...
    function extractScaleLayerParams(prefix) {
        var weights = extractWeightEntry("".concat(prefix, "/scale/weights"), 1);
        var biases = extractWeightEntry("".concat(prefix, "/scale/biases"), 1);
        return { weights: weights, biases: biases };
    }
    function extractConvLayerParams(prefix) {
        var filters = extractWeightEntry("".concat(prefix, "/conv/filters"), 4);
        var bias = extractWeightEntry("".concat(prefix, "/conv/bias"), 1);
        var scale = extractScaleLayerParams(prefix);
        return { conv: { filters: filters, bias: bias }, scale: scale };
    }
    function extractResidualLayerParams(prefix) {
        return {
            conv1: extractConvLayerParams("".concat(prefix, "/conv1")),
            conv2: extractConvLayerParams("".concat(prefix, "/conv2"))
        };
    }
    return {
...
export function extractParamsFromWeigthMap(weightMap) {
    ...
    if (!isTensor2D(fc)) {
        throw new Error("expected weightMap[fc] to be a Tensor2D, instead have ".concat(fc));
    }
    var params = {
